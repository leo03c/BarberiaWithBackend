...
const TUsuarios = () => {
    ...
    const [formData, setFormData] = useState({
        nombre: "", apellidos: "", usuario: "", correo: "", telefono: "", rol: "cliente"
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setEditingId(null);
            setFormData({ nombre: "", apellidos: "", usuario: "", correo: "", telefono: "", rol: "cliente" });
    ...
    return (
        <div className="bg-jetBlack text-lightGray p-6 rounded-lg shadow-lg mt-20">
            <h2 className="text-3xl font-serif font-bold text-mustard mb-6">Gestión de Usuarios</h2>
            ...
            <form onSubmit={handleSubmit} className="bg-gray-800 p-4 rounded-lg shadow mb-6">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <input type="text" name="usuario" placeholder="Usuario" value={formData.usuario} onChange={handleInputChange} className="p-2 bg-gray-700 text-lightGray rounded-md" required />
                    <input type="email" name="correo" placeholder="Correo" value={formData.correo} onChange={handleInputChange} className="p-2 bg-gray-700 text-lightGray rounded-md" required />
                    <input type="tel" name="telefono" placeholder="Teléfono" value={formData.telefono} onChange={handleInputChange} className="p-2 bg-gray-700 text-lightGray rounded-md" required />
                    
                    {/* Selector de rol */}
                    <select name="rol" value={formData.rol} onChange={handleInputChange} className="p-2 bg-gray-700 text-lightGray rounded-md">
                        ...
                        {usuarios.map(user => (
                            <tr key={user.id} className="border-t border-gray-700 hover:bg-gray-700 transition">
                                ...
                                <td className="py-2 px-4 flex justify-center gap-3">
                                    <button onClick={() => handleEdit(user)} className="text-mustard hover:text-yellow-500 transition">
                                        <Pencil size={18} />
                                    </button>
                                    <button onClick={() => handleDelete(user.id)} className="text-red-500 hover:text-red-400 transition">
                                        <Trash2 size={18} />
                                    </button>
                                </td>
                            </tr>
